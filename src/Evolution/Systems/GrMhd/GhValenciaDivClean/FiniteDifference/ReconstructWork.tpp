  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    if (compute_conservatives) {
      ValenciaDivClean::fd::compute_conservatives_for_reconstruction(
          make_not_null(&gsl::at(*vars_on_lower_face, i)),
          eos, fix_to_atmosphere);
      ValenciaDivClean::fd::compute_conservatives_for_reconstruction(
         //Update: now it has too. (BOTH)
          make_not_null(&gsl::at(*vars_on_upper_face, i)),
          eos, fix_to_atmosphere); // I think atmosphere call should go here.
...
void reconstruct_fd_neighbor_work(
  ...
  if (compute_conservatives) {
    ValenciaDivClean::fd::compute_conservatives_for_reconstruction(vars_on_face,
      eos, fix_to_atmosphere); //Missed one here?
  }
}
}  // namespace grmhd::GhValenciaDivClean::fd
